...
watchlist.json 一筆（價格都是**未稅** TWD，澳航日曆只有未稅價）：
  {"route": "TPE-MFM", "since": "2026-09-02", "until": "2026-12-31",
   "threshold": 1700, "newLowMax": 2100, "noNewLow": false, "note": "台北⇄澳門"}
...
from scan import calendar  # noqa: E402
...
def main():
    ...
    for w in watches:
        ...
        if rt not in cal_cache:
            o, d = rt.split("-")
            rows = calendar(o, d)
            cal_cache[rt] = {r["date"]: r["amount"] for r in (rows or [])}
